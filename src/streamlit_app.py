...
CKPT_PATH = Path("models/MalConv2-main/malconvGCT_nocat.checkpoint")
...
def make_chart(data, title, orig_len=None, total_len=None, threshold=1e-5):
    ...
    if len(indices) == 0:
        ...
        pos = values > 0
        neg = ~pos
        bar_w = max(display_len * 0.0026, 25)
        if pos.any():
            fig.add_trace(
                go.Bar(
                    x=indices[pos],
...
def verify_additivity(model, shap_ctx, shap_feat, input_len, logit_val, desc):
    ...
    baseline = torch.zeros((1, input_len, 2 * model.embd_size)).to(device)
    ...
    try:
        with torch.no_grad():
            base_value = model(baseline)[0, 1].item()
    finally:
...
def run_analysis(file_path, status_text=None):
    ...
    if status_text:
        status_text.text(f"[{filename}] Phase 1/3: Original IG...")
    model = create_model()
    orig_t = torch.tensor(orig_bytes.astype(np.int32) + 1, dtype=torch.long).unsqueeze(
        0
    )
    ...
    if status_text:
        status_text.text(f"[{filename}] Phase 3/3: Adversarial IG...")
    model_adv = create_model()
    adv_t = torch.tensor(adv_bytes.astype(np.int32) + 1, dtype=torch.long).unsqueeze(0)
...
def display_result(r):
    ...
    st.plotly_chart(
        make_chart(
            r["ctx_adv"],
            "Context Attribution (Adversarial)",
            orig_len=r["orig_len"],
            total_len=r["total_len"],
        ),
        use_container_width=True,
        key=f"{r['filename']}_ctx_adv",
    )
    st.plotly_chart(
        make_chart(
            r["feat_adv"],
...
def main():
    ...
    if args.file:
        ...
        if not data_dir.is_dir():
            ...
            return
        file_list = sorted(p for p in data_dir.iterdir() if not p.name.startswith("."))
    ...
    for i, fp in enumerate(file_list):
        progress.progress(
            i / len(file_list),
            text=f"Processing {Path(fp).name} ({i + 1}/{len(file_list)})...",
        )
